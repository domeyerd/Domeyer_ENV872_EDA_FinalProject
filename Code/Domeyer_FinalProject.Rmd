...
ggplot() +
  geom_sf(data = counties_sf, color='white', aes(fill = NAME))
...
county.sf <- EPA_data_filtered %>% 
  st_as_sf(coords = c('meanLAT','meanLONG'),
           crs=4326)

county_plot <- ggplot() +
  geom_sf(data = county.sf, color='white', aes(fill = Ozone)) +
  labs(title = "NWIS Gage Locations in Nebraska", subtitle = "Devin Domeyer") +
